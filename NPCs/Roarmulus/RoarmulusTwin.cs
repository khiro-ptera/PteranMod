...
namespace PteranMod.NPCs.Roarmulus
{
	...
	class RoarmulusTwin : ModNPC
	{
		...
		private int frame;
		public static bool twinOneDead = false;
		...
		public override void AI()
		{
			...
			if (ai == 0)
			{
				npc.position.X = player.position.X + 900;
				npc.position.Y = player.position.Y - 300;
			}
			if (player.position.X > npc.position.X || player.position.Y > npc.position.Y + 1000 || player.position.Y < npc.position.Y - 1000)
			{
				...
				if (ai % 1800 == 150)
				{
					...
					for (int i = 0; i < (Main.expertMode ? 5 : 3); i++)
					{
						...
						Projectile.NewProjectile(shootPos.X - 250, shootPos.Y + 8 * j - 200,
							shootVel.X, shootVel.Y * 0.6f, mod.ProjectileType("RoarmulusRocket"),
							(int)(npc.damage * 0.65f), 10f, Main.myPlayer);
					}
				}
				if (ai % 450 == 40) shockBall = 9;
			...
			else if (npc.life <= npc.lifeMax * 0.45f)
			{
				if (ai % 1080 == 150)
				{
					...
					for (int i = 0; i < (Main.expertMode ? 6 : 4); i++)
					{
						int j = i * 10;
						Projectile.NewProjectile(shootPos.X - 250, shootPos.Y + 8 * j - 200,
							shootVel.X, shootVel.Y, mod.ProjectileType("RoarmulusRocket"),
							(int)(npc.damage * 0.95f), 10f, Main.myPlayer);
					}
				}
				if (ai % 360 == 30) shockBall = 12;
		...
		public override void NPCLoot()
		{
			...
			{
				Item.NewItem(npc.getRect(), ItemType<BarkModule>(), Main.rand.Next(2, 4));
			}
			if (!PteranWorld.downedRoar)
			{
				PteranWorld.downedRoar = true;
